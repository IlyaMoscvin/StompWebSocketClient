    ...
    internal class InfoReceiver
    {
        ...
        public async Task<InfoDto> GetInfo()
        {
            ...
            this._config.DefaultHeaders?
                ...
                .ForEach(i =>
                {
                    request.Headers.Add(i.header, i.value);
                });
            UnityEngine.Debug.Log(url);
            foreach (var header in request.Headers)
            {
                UnityEngine.Debug.Log(header.Key +"   "+ header.Value);
            }
            ...
            UnityEngine.Debug.Log($"{nameof(this.GetInfo)}: Finish {content} {info.RoundTripTime}");
            return info;
        }
    }
}